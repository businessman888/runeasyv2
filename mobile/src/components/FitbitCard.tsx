/**
 * Fitbit OAuth connection card for the Settings screen.
 *
 * Mostra status de conexão com Fitbit (via OAuth 2.0 com PKCE) e oferece
...
export function FitbitCard() {
    ...
    const handleConnect = useCallback(async () => {
        setIsBusy(true);
        try {
            const result = await connectWearable('fitbit');
            if (result.success) {
                setIsConnected(true);
    ...
    return (
        <View
            ...
            accessibilityLabel={connected ? 'Fitbit conectado' : 'Fitbit não conectado'}
        >
            <View style={styles.row}>
                <View style={styles.iconContainer}>
                    <MaterialCommunityIcons name="watch-vibrate" size={24} color={DS.cyan} />
                </View>
                ...
                <TouchableOpacity
                    ...
                    accessibilityLabel={connected ? 'Desconectar Fitbit' : 'Conectar Fitbit'}
                    activeOpacity={0.7}
                >
                    {isBusy ? (
                        <ActivityIndicator
