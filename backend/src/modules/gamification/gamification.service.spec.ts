...
describe('GamificationService', () => {
  ...
  let mockNotification: jest.Mocked<
    Pick<NotificationService, 'createNotification' | 'sendPushNotification'>
  >;
  ...
  describe('calculateLevel', () => {
    ...
    it('returns 1 for 999 XP (not enough for level 2)', () =>
  ...
  describe('checkBadges', () => {
    ...
    it('logs a warning for badges with no registered checker (unknown slug)', async () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining('unknown_future_badge'),
      );
    });
      ...
      mockSupabase.from = jest.fn().mockImplementation((table: string) => {
        ...
        if (table === 'user_levels') {
          ...
          chain.single = jest.fn().mockResolvedValue({
            data: { total_points: 0, current_level: 1 },
            error: null,
          });
          chain.upsert = jest
            .fn()
