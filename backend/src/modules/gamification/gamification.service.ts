...
export class GamificationService {
  ...
  private check_pace_on_distance(
    maxPaceMinKm: number,
  ...
  private check_single_duration_min(thresholdMin: number): BadgeChecker {
    return async (
      ...
      const durationSeconds =
        activityData?.elapsed_time ?? activityData?.moving_time ?? 0;
      return durationSeconds / 60 >= thresholdMin;
    };
  }

  private check_activity_count_in_days(
    count: number,
    days: number,
  ): BadgeChecker {
    return async (userId: string): Promise<boolean> => {
  ...
  private check_runs_in_time_window(
    hourFrom: number,
    hourTo: number,
    threshold: number,
  ): BadgeChecker {
  ...
  private async check_all_weekdays(userId: string): Promise<boolean> {
    ...
    const weekdays = new Set(
      activities.map((a) => new Date(a.start_date).getDay()),
    );
  ...
  private async check_fidelidade_plano(userId: string): Promise<boolean> {
    ...
    const completedCount = plannedWorkouts.filter(
  ...
  async getBadges(
    userId: string,
  ): Promise<(Badge & { earned: boolean; earned_at?: string })[]> {
  ...
  private getSaoPauloToday(): string {
    const now = new Date();
    const sp = new Date(
      now.getTime() + this.SAO_PAULO_OFFSET_HOURS * 3600 * 1000,
    );
  ...
  async awardWorkoutXP(
    ...
    if (distanceXP > 0) {
      await this.addPoints(
        userId,
        distanceXP,
        `Distância: ${workout.distance_km.toFixed(1)}km`,
        'workout',
        workout.workoutId,
      );
    ...
    if ((updatedUser?.current_streak || 0) > 1) {
      await this.addPoints(
        userId,
        30,
        `Streak de ${updatedUser.current_streak} dias`,
        'workout',
        workout.workoutId,
      );
      totalAwarded += 30;
    ...
    this.logger.log(
      `User ${userId} awarded ${totalAwarded} XP for workout ${workout.workoutId}`,
    );
    return totalAwarded;
  }
  ...
  async getGlobalRanking(userId: string, limit = 50): Promise<RankingResponse> {
    const [rankingsResult, userResult, countResult] = await Promise.all([
      this.supabaseService
        .from('user_levels')
        .select(
          'total_points, current_level, current_streak, users!inner(id, profile)',
        )
        .gt('total_points', 0)
    ...
    const rankings = (rankingsResult.data || []).map(
  ...
  async getCohortRanking(userId: string, limit = 50): Promise<RankingResponse> {
    ...
    const [rankingsResult, countResult] = await Promise.all([
      this.supabaseService
        .from('user_levels')
        .select(
          'total_points, current_level, current_streak, users!inner(id, profile, created_at)',
        )
