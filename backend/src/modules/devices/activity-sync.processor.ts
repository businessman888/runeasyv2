...
export class ActivitySyncProcessor extends WorkerHost {
  ...
  private async handleSyncActivity(job: Job<WearableActivity>) {
    ...
    try {
      ...
      const result =
        await this.activitySyncService.processWearableActivity(activity);

      this.logger.log(
        `Activity sync result: ${result.action} (${result.activityId})`,
      );
      return result;
    } catch (error) {
      this.logger.error(
  ...
  private async handleSyncBatch(job: Job<{ activities: WearableActivity[] }>) {
    ...
    this.logger.log(
      `Batch complete: ${inserted} inserted, ${skipped} skipped, ${errors} errors`,
    );
   ...
   */
  private async handleFitbitFetch(
    job: Job<{ fitbitUserId: string; date: string }>,
  ) {
    ...
    const { accessToken } = await this.devicesService.getDecryptedToken(
      userId,
      'fitbit',
    );
    ...
    const activities: WearableActivity[] = (data.activities || [])
      .filter(
        (a: any) =>
          ...
          a.activityName?.toLowerCase().includes('walk'),
      )
    ...
    this.logger.log(
      `Fitbit fetch: processed ${activities.length} activities for user ${userId}`,
    );
  ...
  private async handlePolarFetch(job: Job<{ polarUserId: string }>) {
    ...
    const exercises = await this.polarOAuth.listNewExercises(
      accessToken,
      Number(polarUserId),
    );
    ...
    const activities: WearableActivity[] = exercises
      .filter(
        (e: any) =>
          e['detailed-sport-info']?.toLowerCase().includes('running') ||
          e.sport === 'RUNNING',
      )
    ...
    this.logger.log(
      `Polar fetch: processed ${activities.length} exercises for user ${userId}`,
    );
    return { processed: activities.length, results };
  }
  ...
  private normalizeFitbitActivity(raw: any, userId: string): WearableActivity {
    return {
      ...
      moving_time: raw.activeDuration
        ? raw.activeDuration / 1000
        : raw.duration / 1000, // ms to s
      elapsed_time: raw.duration ? raw.duration / 1000 : undefined,
      average_heartrate: raw.averageHeartRate || undefined,
      max_heartrate:
        raw.heartRateZones?.reduce(
          (max: number, z: any) => Math.max(max, z.max || 0),
          0,
        ) || undefined,
