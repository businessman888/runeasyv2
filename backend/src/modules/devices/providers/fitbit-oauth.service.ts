...
export class FitbitOAuthService {
  ...
  private readonly pkceStore = new Map<
    string,
    { verifier: string; userId: string; createdAt: number }
  >();
  ...
  constructor(private readonly configService: ConfigService) {
    this.clientId =
      this.configService.get<string>('FITBIT_CLIENT_ID') || '23VCWS';
    this.clientSecret =
      this.configService.get<string>('FITBIT_CLIENT_SECRET') || '';
    this.redirectUri =
      this.configService.get<string>('FITBIT_REDIRECT_URI') ||
  ...
  async exchangeCode(
    code: string,
    state: string,
  ): Promise<{ tokens: FitbitTokenResponse; userId: string }> {
    ...
    this.logger.log(
      `Fitbit tokens obtained for user ${pkceEntry.userId} (fitbit_uid: ${tokens.user_id})`,
    );
