...
export class PolarOAuthService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.clientId =
      this.configService.get<string>('POLAR_CLIENT_ID') ||
      'c38ed64d-d8b5-4367-a463-94ff8bd673c7';
    this.clientSecret =
      this.configService.get<string>('POLAR_CLIENT_SECRET') || '';
    this.redirectUri =
      this.configService.get<string>('POLAR_REDIRECT_URI') ||
  ...
  async exchangeCode(
    code: string,
    state: string,
  ): Promise<{ tokens: PolarTokenResponse; userId: string }> {
    ...
    if (!response.ok) {
      ...
      this.logger.error(
        `Polar token exchange failed: ${response.status} ${errorBody}`,
      );
    ...
    this.logger.log(
      `Polar tokens obtained for user ${stateEntry.userId} (polar_uid: ${tokens.x_user_id})`,
    );
  ...
  async registerUser(accessToken: string, polarUserId: number): Promise<void> {
    ...
    if (response.ok || response.status === 409) {
      this.logger.log(
        `Polar user ${polarUserId} registered in AccessLink (status: ${response.status})`,
      );
      return;
    }
    ...
    const errorBody = await response.text();
    this.logger.error(
      `Polar AccessLink registration failed: ${response.status} ${errorBody}`,
    );
  ...
  async listNewExercises(
    ...
    if (!txResponse.ok) {
      ...
      this.logger.error(
        `Polar exercise transaction failed: ${txResponse.status} ${errorBody}`,
      );
      throw new Error(
        `Polar exercise transaction failed: ${txResponse.status}`,
      );
    }
    ...
    this.logger.log(
      `Fetched ${exercises.length} new Polar exercises for user ${polarUserId}`,
    );
    return exercises;
  }
