...
export class WellnessService {
    ...
    async getSummary(userId: string): Promise<WellnessSummaryResponseDto> {
        ...
        const readiness = this.buildReadinessBlock(readinessStatus, readinessAnswers);
     ...
     */
    private getWeekWindow(today: Date, weekOffset: number): { start: Date; end: Date } {
    ...
    private async fetchTodayReadinessAnswers(
        userId: string,
    ): Promise<{
        sleep: number;
        legs: number;
    ...
    private async getUserProfileSafe(
        userId: string,
    ): Promise<{ birth_date?: { day: number; month: number; year: number } } | null> {
        try {
    ...
    private buildPerformanceBlock(
        ...
        const distanceCurr = sum(currentActivities, (a) => (a.distance || 0) / 1000);
        const distancePrev = sum(previousActivities, (a) => (a.distance || 0) / 1000);
        ...
        return {
            distance: this.buildMetric(
                round2(distanceCurr),
                round2(distancePrev),
                this.sparkline7(currentActivities, currentWeek.start, (a) => (a.distance || 0) / 1000),
            ),
            ...
            pace: this.buildMetric(
                Math.round(paceCurr),
                Math.round(pacePrev),
                this.sparkline7(currentActivities, currentWeek.start, (a) => a.average_pace || 0),
            ),
            duration: this.buildMetric(
                Math.round(durCurr),
                Math.round(durPrev),
                this.sparkline7(currentActivities, currentWeek.start, (a) => (a.moving_time || 0) / 60),
            ),
            calories: this.buildMetric(
                Math.round(calCurr),
                Math.round(calPrev),
                this.sparkline7(currentActivities, currentWeek.start, (a) => a.calories || 0),
            ),
            elevation: this.buildMetric(
                Math.round(elevCurr),
                Math.round(elevPrev),
                this.sparkline7(currentActivities, currentWeek.start, (a) => a.elevation_gain || 0),
            ),
        };
    }
    ...
    private buildZonesBlock(
        ...
        const total = minutes.Z1 + minutes.Z2 + minutes.Z3 + minutes.Z4 + minutes.Z5;
    ...
    private buildHealthBlock(
        ...
        const recoveryHrs = last8wActivities
            .filter((a) => a.type === 'recovery' || a.type === 'easy_run' || a.type === 'rodagem')
            .map((a) => a.average_heartrate)
            .filter((v): v is number => typeof v === 'number' && v > 0);
    ...
    private buildEvolutionBlock(
        ...
        const statsByWeek = new Map(
            weeklyStats.map((s) => [s.week_start, s]),
        );
