...
export class QuestionSetsParserService implements OnModuleInit {
  ...
  async onModuleInit() {
    this.logger.log(
      '[QuestionSetsParser] onModuleInit started - deferring heavy loading to background',
    );
  ...
  private async preloadFromDatabase(): Promise<void> {
    try {
      ...
      if (data && data.length > 0) {
        ...
        this.logger.log(
          `[QuestionSetsParser] Preloaded ${this.questionSets.length} sets from database`,
        );
      }
    } catch (error: any) {
  ...
  async getQuestionSetForUser(userId: string): Promise<QuestionSet> {
    this.logger.log(
      `[QuizSelection] Selecting question set for user ${userId}`,
    );
    ...
    try {
      ...
      if (historyError) {
        this.logger.error(
          `[QuizSelection] Error fetching history: ${historyError.message}`,
        );
      ...
      this.logger.log(
        `[QuizSelection] User ${userId} has responded to ${respondedCount} sets: [${Array.from(respondedSetNumbers).join(', ')}]`,
      );

      this.logger.log(
        `[QuizSelection] User ${userId} has responded to ${respondedCount} sets`,
      );
      ...
      if (setsError || !allSets?.length) {
        this.logger.error(
          `[QuizSelection] Error fetching question sets: ${setsError?.message}`,
        );
        return this.getFallbackSet();
      }

      // 3. Find first unresponded set
      const unrespondedSet = allSets.find(
        (s) => !respondedSetNumbers.has(s.set_number),
      );

      if (unrespondedSet) {
        this.logger.log(
          `[QuizSelection] User ${userId} has responded ${respondedCount} sets. Delivering NEW Set #${unrespondedSet.set_number} ("${unrespondedSet.set_name}")`,
        );
      ...
      if (oldestResponded) {
        const lruSet = allSets.find(
          (s) => s.set_number === oldestResponded.set_number,
        );
        if (lruSet) {
          this.logger.log(
            `[QuizSelection] User ${userId} completed all ${this.TOTAL_SETS} sets. Reusing LRU Set #${lruSet.set_number} (last used: ${oldestResponded.created_at})`,
          );
      ...
      this.logger.warn(
        `[QuizSelection] Fallback to first available set for user ${userId}`,
      );
      return this.mapDbToQuestionSet(allSets[0]);
    } catch (error: any) {
      this.logger.error(
        `[QuizSelection] Unexpected error for user ${userId}: ${error?.message}`,
      );
      return this.getFallbackSet();
    }
  }
  ...
  async loadQuestionSets(): Promise<void> {
    try {
      ...
      if (!fs.existsSync(filePath)) {
        this.logger.warn(
          `Question sets file not found at ${filePath}, using fallback questions`,
        );
        this.questionSets = this.getFallbackSets();
        return;
      }
  ...
  private parseMarkdownContent(content: string): QuestionSet[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (setMatch) {
        ...
        this.logger.log(
          `>>> Detected Set ${newSetNumber} header at line ${i + 1}`,
        );
        ...
        if (currentSet && currentQuestions.length >= 5) {
          ...
          this.logger.log(
            `    Saved Set ${currentSet.setNumber} (${currentQuestions.length} questions)`,
          );
        } else if (currentSet) {
          this.logger.warn(
            `    Set ${currentSet.setNumber} SKIPPED - only had ${currentQuestions.length} questions`,
          );
        }
      ...
      if (questionMatch && currentSet) {
        ...
        const questionText =
          questionMatch[2]?.trim() || this.getDefaultQuestion(category);
    ...
    if (currentSet && currentQuestions.length >= 5) {
      ...
      this.logger.log(
        `    Saved final Set ${currentSet.setNumber} (${currentQuestions.length} questions)`,
      );
    } else if (currentSet) {
      this.logger.warn(
        `    Final Set ${currentSet.setNumber} SKIPPED - only had ${currentQuestions.length} questions`,
      );
    }
  ...
  private parseInitialSet(
    ...
    for (let i = 0; i < lines.length && questionIndex < 5; i++) {
      ...
      if (questionMatch) {
        ...
        const questionText =
          questionMatch[3]?.trim() || this.getDefaultQuestion(category);
  ...
  getSetNumberForDay(date: Date): number {
    ...
    const availableSets =
      this.questionSets.length > 0 ? this.questionSets.length : this.TOTAL_SETS;
    const setIndex = seed % availableSets;
    const setNumber = this.questionSets[setIndex]?.setNumber || setIndex + 1;

    this.logger.log(
      `[QuestionSetsParser] Date: ${dateString} (São Paulo) → Set ${setNumber} (seed: ${seed}, index: ${setIndex})`,
    );
