...
export class UsersService {
  ...
  async getUser(userId: string) {
    const { data, error } = await this.supabaseService
      .from('users')
      .select(
        'id, email, profile, subscription_status, created_at, onboarding_completed',
      )
      .eq('id', userId)
      .single();
  ...
  async syncGoogleMetadata(userId: string): Promise<void> {
    try {
      const {
        data: { user: authUser },
        error,
      } = await this.supabaseService.auth.admin.getUserById(userId);
      if (error || !authUser) {
        this.logger.warn(
          `Could not fetch auth user ${userId} for metadata sync`,
        );
        return;
      }
      ...
      this.logger.error(
        `Failed to sync Google metadata for user ${userId}`,
        err,
      );
    }
  }
  ...
  async syncOnboardingBiometrics(userId: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to sync onboarding biometrics for user ${userId}`,
        err,
      );
    }
  }
  ...
  async markOnboardingComplete(userId: string) {
    ...
    if (error) {
      this.logger.error(
        `Failed to mark onboarding complete for user ${userId}`,
        error,
      );
      throw error;
    }
  ...
  async uploadAvatar(
    ...
    try {
      ...
      this.logger.warn(
        `Avatar cleanup skipped for ${userId}: ${(err as Error).message}`,
      );
    }
