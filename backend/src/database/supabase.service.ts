  ...
  onModuleInit() {
    ...
    try {
      ...
      if (!supabaseUrl || !supabaseKey) {
        this.logger.error(
          '[SupabaseService] ❌ Missing Supabase configuration!',
        );
        throw new Error(
      ...
      this.logger.log(
        '[SupabaseService] ✅ Supabase clients initialized (db + auth isolated)',
      );
    } catch (error: any) {
      this.logger.error(
        `[SupabaseService] ❌ Initialization failed: ${error?.message}`,
      );
      throw error;
    }
  }
